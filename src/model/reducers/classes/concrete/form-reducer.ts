...
import type { FormReducerConstructorArgs, FormReducerState, UserDefinedOrDefaultAdapter } from '../../types';
...
> extends AbstractFormReducer<FormElements, Adapters> {
  private stateManager : AbstractStateManager<FormReducerState<FormElements, Adapters>>;
  ...
  private adapters : ReadonlyArray<UserDefinedOrDefaultAdapter<FormElements, Adapters>>;
  private transientFormElements : ReadonlyArray<Extract<FormElements[number], PossiblyTransient<true>>>;
  ...
  public constructor({
    ...
    super();
    this.valueReducer = new ValueReducer<FormValue<FormElements, Adapters>>({ members : adapters });
    this.validityReducer = new FormValidityReducer({
      adapters,
      transientFormElements,
      groups
    });
    this.stateManager = new StateManager<FormReducerState<FormElements, Adapters>>({
  ...
  public subscribeToState(cb: (state: FormReducerState<FormElements, Adapters>) => void): Subscription {
  ...
  private subscribeToTransientFormElements() : void {
    this.transientFormElements.forEach(element => {
      element.subscribeToState(state => {
        this.validityReducer.processTransientElementStateUpdate(element.name, state);
        this.state = {
          ...this.state,
          validity : this.validityReducer.validity
        };
