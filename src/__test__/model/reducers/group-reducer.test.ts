...
describe('GroupReducer', () => {
  ...
  test(`Its validity defaults to Validity.Pending there are pending included 
  members but no invalid included members.`, () => {
  ...
  test(`Its validity defaults to Validity.Invalid if at least one included 
  member's validity is Validity.Invalid.`, () => {
    const promiseScheduler = new PromiseScheduler();
    const requiredAsync = new AsyncValidator<string>({
      predicate: (value): Promise<boolean> => {
        return promiseScheduler.createScheduledPromise(value.length > 0);
