  ...
  test('Its default value is the result of running the provided derive function against the state of its sources.', () => {
    ...
    const derivedValue = new DerivedValue({ 
      ...
      derive : ([color1, color2]):string => {
        if(!(color1 in complementaryColors) || !(color2 in complementaryColors)) {
          return messages.notAColor;
        }
        return complementaryColors[color1 as keyof typeof complementaryColors] === color2 ?
          messages.complementary : messages.nonComplementary
      }
    });
