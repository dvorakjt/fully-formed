...
describe('DefaultExcludableAdapter', () => {
  ...
  test(`The exclude property of its state matches that of its source's 
  state.`, () => {
    const source = new ExcludableField({
      name: 'testField',
      defaultValue: '',
  ...
  test(`When the exclude property of its source's state changes, its own 
  state's exclude property is updated.`, () => {
    const source = new ExcludableField({
      name: 'testField',
      defaultValue: '',
      excludeByDefault: true,
